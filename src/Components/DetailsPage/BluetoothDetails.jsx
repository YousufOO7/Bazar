...
const BluetoothDetails = () => {
    ...
    // add cart 
    const handleAddToCard = bluetoothDetails => {
        if (user && user?.email) {
            const cartItem = {
                productId: bluetoothDetails._id,
                model: bluetoothDetails?.model,
                email: user?.email,
                price: parseFloat(bluetoothDetails?.cashDiscountPrice),
                image: bluetoothDetails?.mainImage
            }
                ...
                .then(res => {
                    ...
                    if (res.data.insertedId) {
                        Swal.fire({
                            position: "center",
                            icon: "success",
                            title: `${bluetoothDetails?.model} added to the cart`,
            ...
            <div className="bg-white px-4 lg:px-16 h-full py-5">
                ...
                <section className="md:flex gap-5">
                    ...
                    <div key={bluetoothDetails._id} className="mt-5 space-y-5">
                        ...
                        <div >
                            <button onClick={() => handleAddToCard(bluetoothDetails)} className="px-2 p-1 border border-[#ff882a] text-[#ff882a] text-xs lg:text-sm rounded-md hover:bg-[#ff882a] hover:text-white transition duration-700 ease-in-out">Add To Cart</button>
                        </div>
                    </div>
                </section>
